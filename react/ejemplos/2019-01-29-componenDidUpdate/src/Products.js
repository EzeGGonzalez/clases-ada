...
class Products extends Component {
  ...
  // esta funcion se ejecuta cuando alguna props o el state del componente cambia
  // también se ejecuta cuandl la URL cambia (por ejemplo, si cambian los query params)
  componentDidUpdate () {
  ...
  render () {
    ...
    return (
      <ul>
        {this.state.cosas.map(c => <Link to={`/items/${c.id}`}><li>{c.title}</li></Link>)}
      </ul>
    )
  }
}
